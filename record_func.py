...
def record_write_new_json():
    uuid=uuid4_generate()
    recordStatus=record_obj.status_record.NEW.value
    created=date_time_generate()
    print(uuid,recordStatus,created)
    record = record_obj.record_recordID_create(uuid,recordStatus,created,"","","")
...
def json_record_deserialize(record):
    x = json.loads(record, object_hook=lambda d: namedtuple('X', d.keys())(*d.values()))
    print(x.recordId, x.info.recordStatus, x.info.created, x.info.updated,x.info.deleted, x.info.recordData)
    record = record_obj.record_recordID_create(x.recordId,x.info.recordStatus,x.info.created,x.info.updated,x.info.deleted,x.info.recordData)
    return record
...
def record_uuid_delete(uuid):
    ...
    for i in range(len(records_list)):
        ...
        if record.recordId == uuid:
            ...
            record.info.recordStatus = record_obj.status_record.DELETED.value
...
def record_uuid_patch(uuid):
    ...
    for i in range(len(records_list)):
        ...
        if record.recordId == uuid:
            #JsonPatch
            found_record_index = i
            record.info.recordStatus = record_obj.status_record.UPDATED.value

            if (record.info.updated == ""):
                record.info.updated = date_time_generate()
            else:
                record.info.updated = record.info.updated + "," + date_time_generate()
